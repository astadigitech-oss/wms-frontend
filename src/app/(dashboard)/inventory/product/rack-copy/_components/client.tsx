...
export const Client = () => {
  ...
  const { data: dataSummarySaldo, isLoading: isLoadingSummarySaldo } =
  ...
  const saldoAwal: any = useMemo(() => {
    return summarySaldo?.saldo_awal;
  }, [summarySaldo]);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 w-full mb-6">
        <div className="bg-sky-100 shadow rounded-xl p-5 flex flex-col border border-gray-200">
          <div className="flex items-start justify-between gap-3">
            <div>
              <h4 className="text-sm text-gray-500">Saldo Awal Display</h4>
              <p className="text-xs text-gray-400 mt-1">
                As of: {formatDateTime(summaryAsOf)}
              </p>
            </div>
            {!saldoAwal && !isLoadingSummarySaldo ? (
          ...
          <p className="text-3xl font-bold mt-4">
            {isLoadingSummarySaldo ? "-" : formatNumber(saldoAwal?.qty)}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4 text-sm">
            <div>
              <p className="text-gray-500">Total Price</p>
              <p className="font-semibold text-gray-900">
                {isLoadingSummarySaldo
                  ? "-"
                  : formatRupiah(saldoAwal?.total_price)}
              </p>
            </div>
            <div>
              ...
              <p className="font-semibold text-gray-900">
                {isLoadingSummarySaldo
                  ? "-"
                  : formatRupiah(saldoAwal?.total_price_before)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-sky-100 shadow rounded-xl p-5 flex flex-col border border-gray-200">
          ...
          <p className="text-3xl font-bold mt-4">
            {isLoadingSummarySaldo ? "-" : formatNumber(saldoRealtime?.qty)}
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4 text-sm">
            <div>
              <p className="text-gray-500">Total Price</p>
