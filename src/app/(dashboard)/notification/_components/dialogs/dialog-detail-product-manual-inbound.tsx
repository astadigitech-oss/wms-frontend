...
import { useActionProductMI } from "../../_api/use-approve-product-mi";
...
export const DialogDetailProductManualInbound = ({
  open,
  onCloseModal,
  baseData,
  miId,
  ...
  const { mutate: mutateAction, isPending } = useActionProductMI();
  ...
  const detail = resource?.detail_product ?? {};
            ...
            <div className="border border-sky-400/80 rounded-md p-4 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-4 gap-4 text-sm">
                <div>
                  ...
                  <p className="font-semibold">{detail.new_name_product}</p>
                </div>

                <div>
                  <p className="text-xs">Price</p>
                  <p className="font-semibold">
                    {formatRupiah(detail.new_price_product)}
                  </p>
                </div>

                <div>
                  <p className="text-xs">Qty</p>
                  <p className="font-semibold">{detail.new_quantity_product}</p>
                </div>

                <div>
                  <p className="text-xs">Status</p>
                  <p className="font-semibold">{detail.new_status_product}</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 border-t pt-4">
                <NodeForm
                  label="Old Price"
                  value={formatRupiah(detail.old_price_product)}
                />
                <NodeForm
                  label="Category"
                  value={detail.new_category_product}
                />
                <NodeForm label="Tag" value={detail.new_tag_product} />

                <NodeForm label="Lolos" value={detail.new_quality?.lolos} />
                <NodeForm label="Damaged" value={detail.new_quality?.damaged} />
                <NodeForm
                  label="Abnormal"
                  value={detail.new_quality?.abnormal}
                />
              </div>
            </div>
          </div>
